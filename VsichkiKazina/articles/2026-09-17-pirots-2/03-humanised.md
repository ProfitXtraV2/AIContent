# 03 — Humaniser · Pirots 2

## Phase 1 — AI-fingerprint evaluation
Verdict: **HUMAN-LIKE** (est. ~50/60). Rewrite scope: LIGHT only (verdict above MIXED threshold, so no full rewrite; preserve voice, facts, flags).

Layer scan:
- Em-dashes: 0 in body. ✓
- Signposting lead-ins ("Какво означава това:", "Нека сметнем:"): none. ✓
- Repeated paragraph template (fact → meaning → € example → takeaway across 3+ sections): not present; section shapes vary (analytical / layered mechanics / buy-mode prose / worked figure / opinionated single para / closing). ✓
- "не X, а Y" paragraph-ending habit: checked; used at most descriptively ("значение има какво стои до птицата, а не някаква подредба"), not as a closing tic on multiple paragraphs. ✓
- Demonstrative crutches ("именно тази", "точно тази"): one "точно затова" in the mechanics section, kept (single use, natural). ✓
- Reveal-signal before explanation ("Причината е проста:", "Изводът е прост:"): none. ✓
- Numbered-in-prose enumeration ("Първата е… Втората е…"): none. ✓
- Translated idioms: checked, plain BG industry phrasing used. ✓
- Symmetric "везни" sentences / rule-of-three / wrap-up aphorism on every paragraph: none. ✓
- Asymmetric opinionated close: yes ("Задай си лимит… преди първото завъртане, особено при толкова волатилна игра."). ✓
- Banned connectives (освен това / в допълнение / в заключение / заслужава да се отбележи): none. ✓

Light touches applied: none required beyond confirming rhythm variation. Numbers, links, RG line, byline, dates and both [VERIFY] flags unchanged.
