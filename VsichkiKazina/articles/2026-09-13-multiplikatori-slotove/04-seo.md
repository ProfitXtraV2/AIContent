...
## SEO AUDIT
Target query: Мултипликатори (множители) в ротативките: как работят и кога умножават | Intent: informational (mechanic hub) | Format match: yes — definitional-first, mechanism before examples, honest-angle payoff.

Relevance score: 92/100
(Query alignment 24/25 | Entity coverage 33/35 | Subtopic completeness 22/25 | On-page 13/15)

Query appears in: H1, first sentence, one H2 ("Видовете, които ще срещнеш" + "как умножават" satisfied in body), meta, title. Once each — no over-repetition.

MISSING ENTITIES (added through substance, not name-drops): secondary terms already covered — множител-wild (S3 символ-множител ×N), множител в безплатни завъртания (S5 + S7), нарастващ множител (S4), случаен множител (S5), множител на печалбата (S2/S3). Added natural coverage of "домашно предимство" phrasing in S6 to strengthen the RTP/edge relation.
SEMANTIC GAPS: none material for the scope. PAA-style questions answered: what a multiplier multiplies (S2), types (S3), where big multipliers occur (S5), whether a bigger multiplier is "better" (S6), whether bonus-multiplier winnings are withdrawable (S7).
FLOODING/OVER-OPTIMISATION DEFECTS: none — primary term in ≤2 headings; synonym field (коефициент / множител / x-число) used naturally.
ON-PAGE FIXES: added hero image + two infographic references with descriptive Bulgarian ALT and captions (data-placement in S3 and S6). Title tag and meta set below.
INTERNAL LINKS (kept, all from the approved live set, 4 in-body + RG footer):
- [речника с казино понятия → /blog/rechnik-kazino-termini/] (S3)
- [Sweet Bonanza → /blog/sweet-bonanza/] (S5)
- [прогресивния джакпот → /blog/progresivni-dzhakpoti/] (S6)
- [ръководството за превъртане → /blog/kak-raboti-razigravaneto/] (S7)
- [инструментите за отговорна игра → /otgovorna-igra/] (RG)
SCHEMA: Article + Person (author Георги Тодоров). No FAQPage (no genuine FAQ block). No Review schema (guide, not review).
...
## CHANGE SUMMARY
- Set title tag (≤60) and meta description (≤155); primary query once in each, no flooding.
- Added hero + two infographic references with descriptive Bulgarian ALT and captions at the two data spots (S3 types overview, S6 RTP/volatility reality).
- Strengthened the RTP/edge relation in S6 with the phrase „домашното предимство остава непокътнато" (entity coverage), in the same voice, no new figure.
- All four in-body internal links from the approved live set preserved; /otgovorna-igra/ in RG.
- Every figure preserved exactly (diffed against Step 3): 1x/2x/3x/5x, 3x/6x/9x/15x, 95.97%, 2x–100x, 500x, 6×5, 8+, €0.20/€0.40/€20/€0.10/€0.40. No em-dash introduced.
