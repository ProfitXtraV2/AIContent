# 05-GATE-REPORT — Всички Казина · vk-0010

## BRAND COMPLIANCE SCORECARD — „Как работят кешбекът и лоялните програми в казиното"
Verdict: PASS WITH FIXES
Total: 91/100
Personality 18/20 | Tone 14/15 | E-E-A-T 18/20 | Trust signals 13/15 | Language&Style 14/15 | RG 14/15

### CRITICAL (blocks publish)
- TRUST: required footer (byline + dates + About slot + RG block + affiliate disclosure) was not yet
  appended to the article body. FIX APPLIED in Phase 2 (verbatim footer block added). No other criticals.

### MODERATE
- Brand-name mention: body carries one natural implicit mention („в нашата методология за оценяване");
  within the 2–3 max, no promotional tone. Acceptable, no fix.
- „Дотук просто." + „двама играчи… единият върти слот игри, а другият играе блекджек" reads human; no
  two-beat staccato dominance. No fix.

### VERIFY QUEUE (route to human)
- None. Concept guide: no operator, no T&C, no НАП licence №, no tax claim, no live offer. Zero flags.

### VOICE NOTES (protect during fixes)
- Neutral editorial „ние"/second-person register; asymmetric opinionated close; native BG vocabulary.
  Footer insertion only; body prose untouched.

### MATHS RECALC (all illustrative, all correct)
- 10% × €200 нетна загуба = €20. ✓
- €20 кешбек × 3x (база: само кешбекът) = €60 залози преди теглене. ✓
- 10% кешбек → задържана загуба 90% от периода (100% − 10%). ✓
- Illustrative точки: 1 точка / €10 заложени; 100 точки = €1. Internally consistent. ✓
- Превъртане base stated at every mention (само сумата на кешбека / не депозит+кешбек / различна база =
  различно изискване). ✓

### COMPLIANCE CHECKS
- Zero em-dashes in body, title, meta. ✓
- Banned promise/hype words: none. Banned AI connectives (освен това / в допълнение / в заключение / …):
  none. ✓
- Internal links: 4, all from approved set. ✓
- Verbatim „18+ Хазартът може да пристрасти. Играйте отговорно." present in body RG touch. ✓
- Byline Георги Тодоров; brand name exactly „Всички Казина"; affiliate disclosure + pending-licence
  footer verbatim; RG signposting to /otgovorna-igra/ + регистър на уязвимите лица (НАП). ✓ (post-fix)

NUMBER DIFF 04→05: identical (footer added only; no body figure changed).
