...
Byline: Editorial Team | Market: BG — provider-hub guide (public company/game data); routed editorial because it is a neutral provider profile, not a testing review. Published byline is always Георги Тодоров.
...
---
CANON ADDITIONS: none (editorial mode, no persona anecdote).
[DATA NEEDED]: none. Every RTP / max-win / year sourced to a fetched page (see 00-brief SOURCES). Two figures carried as source-disagreements, not assertions: Buffalo Blitz max win (racingpost 300x vs up to 10 000x elsewhere) and Great Blue RTP (94.30% vs 96.03%) — both used to illustrate the multi-build reality, hedged in prose.
